...
class WorkerExtension:
    ...
    def receive_weight_update(self: LikeWorker, request: WeightUpdateRequest | list[dict[str, Any]]):
        torch.cuda.synchronize(self.device)
        logger.info("Start receiving weight update")
        if isinstance(request, list):
            parameters_info = request
        else:
            parameters_info = [info.model_dump() for info in request.parameters_info]
        for info in parameters_info:
            name = info["name"]
            shape = info["shape"]
            dtype = info["dtype"]
            model_dtype = self.model_config.dtype
            assert dtype == str(model_dtype), (
                f"mismatch dtype: src {dtype}, dst {self.model_config.dtype}"
            )
            buffer = torch.empty(tuple(shape), dtype=model_dtype, device=self.device)
            torch.distributed.broadcast(buffer, src=0, group=self.process_group)
            loaded_params = self.model_runner.model.load_weights(weights=[(name, buffer)]) # type: ignore
            if len(loaded_params) != 1:
                raise ValueError(f"model {name} not found in model state dict")
...
class WeightUpdateManager:
    ...
    async def receive_weight_update(self, request: WeightUpdateRequest):
        # Convert Pydantic model to serializable data for vllm>=0.10.1 compatibility
        parameters_info = [
            {
                "name": info.name,
                "shape": info.shape,
        ...
        print(f"Receiving weight update for {len(parameters_info)} parameters")
        await self.engine_client.collective_rpc_async(
